<template>
  <div class="container">
    <div class="flex-column width-10 align-self-start mg-bot-20 box-shadow"
      ...
      <div class="flex-row flex-wrap space-between blue width-10 padding-10-20 align-items-center">
        ...
        <div class="flex-row align-items-center size-text" v-if="showUpdate">
          <div class="btn mg-l-10 mg-r-10" @click="startReplyUpdate">
            <p>редактировать</p>
          </div>
          <div class="btn mg-l-10 mg-r-10" @click="replyDelete">
            <p>удалить</p>
          </div>
        </div>
      </div>
    ...
    <form class="flex-column bg-violet width-9 mg-rl-auto box-shadow padding-20-40" @submit.prevent="submit" v-else>
      ...
      <p class="btn-pink size-text align-self-center mg-top-20" @click="cancelReplyUpdate">отменить</p>
    </form>
  </div>
</template>
    ...
    methods: {
      ...
      replyDelete() {
        if (+window.localStorage.getItem('id') === +this.data.user_id || this.$auth.isEditor()) {
          axios.delete(`https://api.dihauti.ru/api/questions/${this.$route.params.slug}/replies/${this.data.id}`)
            .then(() => {
              swal.fire('Отлично!', 'Сообщение удалено', 'success')
                .then(() => {
      ...
      like() {
        if (this.$auth.isAuthenticated) {
          axios.post(`https://api.dihauti.ru/api/questions/${this.$route.params.slug}/replies/${this.data.id}/like`)
            .then(res => {
              if (res.status === 201) {
